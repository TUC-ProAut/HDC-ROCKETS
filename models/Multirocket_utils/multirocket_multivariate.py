...
def transform(X, X1, parameters, parameters1, n_features_per_kernel=4):
    ...
    # debug mode for pooling analysis
    use_ppv = True
    use_lspv = True
    use_mpv = True
    use_mipv = True
            ...
            for kernel_index in range(num_kernels):
                ...
                if _padding1 == 0:
                    for feature_count in range(num_features_this_dilation):
                        ...
                        if use_ppv: features[example_index, end] = ppv / C.shape[0]
                        end = end + num_features
                        if use_lspv: features[example_index, end] = max_stretch
                        end = end + num_features
                        if use_mpv: features[example_index, end] = mean / ppv if ppv > 0 else 0
                        end = end + num_features
                        if use_mipv: features[example_index, end] = mean_index / ppv if ppv > 0 else -1
                else:
                    ...
                    for feature_count in range(num_features_this_dilation):
                        ...
                        if use_ppv: features[example_index, end] = ppv / _c.shape[0]
                        end = end + num_features
                        if use_lspv: features[example_index, end] = max_stretch
                        end = end + num_features
                        if use_mpv: features[example_index, end] = mean / ppv if ppv > 0 else 0
